...
- Supabase Storage를 도입하면 인증, 가족 접근 제어, 원격 파일 삭제 정책까지 함께 결정해야 하므로 별도 PR에서 다루는 편이 안전합니다.

제약:

- 선택한 로컬 URI의 장기 유지 여부는 플랫폼 동작에 의존합니다.
- 앱 삭제, 기기 분실, 다중 기기 동기화는 해결하지 않습니다.
- 실제 가족 공유, 원격 백업, 안정적 파일 보관이 필요해지면 Supabase Storage 또는 파일 시스템 저장 전략을 다시 결정합니다.
